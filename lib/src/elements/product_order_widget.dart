...
class _ProductOrderWidgetState extends State<ProductOrderWidget> {
  ...
  Widget _addMoreProductButton() {
    return Row(
      ...
      children: [
        if (widget.deleteCallback == null || widget.orderModel!.status != AppConfig.orderStatusData[1]["id"])
          GestureDetector(
                  ...
                  child: Row(
                    children: [
                      ...
                      GestureDetector(
                        child: Icon(
                          Icons.remove,
                          color: (widget.orderModel!.status != AppConfig.orderStatusData[1]["id"] || widget.productOrderModel!.couponQuantity == 1)
                              ? Colors.grey
  ...
  Widget _priceWidget() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        if (widget.productOrderModel!.productModel!.id == null && (widget.productOrderModel!.productModel!.price == 0))
          _addPriceButton()
        else
          Row(
            mainAxisAlignment: MainAxisAlignment.start,
            children: [
              if (widget.productOrderModel!.productModel!.id == null && widget.orderModel!.status == AppConfig.orderStatusData[1]["id"])
                _editPriceButton()
              else
                SizedBox(),
