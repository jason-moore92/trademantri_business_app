...
class ProductPriceOldDialog {
  static show(
    ...
    Map<String, dynamic> _productData = Map<String, dynamic>();

    productData!.forEach((key, value) {
      if (key != "imageFile") {
        _productData[key] = value;
      }
    });

    _priceController.text = _productData["price"] != null ? _productData["price"].toString() : "";
    _discountController.text = _productData["discount"] != null ? _productData["discount"].toString() : "";
    _productData["taxPercentage"] = _productData["taxPercentage"] ?? 0;
    ...
    void _submit(double totalPrice) {
      if (_formkey.currentState!.validate()) {
        ...
        if (callback != null) {
          double tax = (_productData["price"] - (_productData["discount"] ?? 0)) *
              (_productData["taxPercentage"] != null ? _productData["taxPercentage"] : 0) /
              (100 + (_productData["taxPercentage"] != null ? _productData["taxPercentage"] : 0));

          _productData["discount"] = _productData["discount"] ?? 0;
          _productData["promocodeDiscount"] = _productData["promocodeDiscount"] ?? 0;
          _productData["taxPrice"] = double.parse(tax.toStringAsFixed(2));
          _productData["priceFor1OrderQuantityBeforeTax"] = _productData["price"];
          _productData["priceFor1OrderQuantityAfterTax"] = _productData["price"] + _productData["taxPrice"];
          callback(_productData);
        }
      }
    }
      ...
      builder: (context) {
        return Consumer<RefreshProvider>(builder: (context, refreshProvider, _) {
          ...
          if (_productData["price"] != null) {
            totalPrice = (_productData["price"] - (_productData["discount"] ?? 0)) *
                (1 + (_productData["taxPercentage"] != null ? _productData["taxPercentage"] / 100 : 0));
            // _productData["orderQuantity"];
                  ...
                  children: <Widget>[
                    Row(
                      ...
                      children: [
                        ...
                        Expanded(
                          child: Text('${_productData["name"]}', style: TextStyle(fontSize: fontSp * 14, color: Colors.black)),
                        ),
                      ],
                    ),
                    ...
                    //     Text('${_productData["orderQuantity"]}', style: TextStyle(fontSize: fontSp * 14, color: Colors.black)),
                    //   ],
                    // ),
                    SizedBox(height: heightDp * 5),
                    Row(
                        ...
                        Expanded(
                          ...
                          child: Container(
                            ...
                            child: TextFormField(
                              ...
                              onChanged: (input) {
                                if (input.isNotEmpty) _productData["price"] = double.parse(input.trim());
                                refreshProvider.refresh();
                              },
                              onSaved: (input) => _productData["price"] = double.parse(input!.trim()),
                            ...
                            Expanded(
                              ...
                              child: Container(
                                ...
                                child: TextFormField(
                                  ...
                                  inputFormatters: [FilteringTextInputFormatter.allow(RegExp(r"^\d+\.?\d{0,2}"))],
                                  onChanged: (input) {
                                    if (input.isNotEmpty) _productData["discount"] = double.parse(input.trim());
                                    refreshProvider.refresh();
                                  },
                                  onSaved: (input) => _productData["discount"] = double.parse(input!.trim()),
                                  validator: (input) => input!.trim().isEmpty
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        Text(
                          _productData["price"] == null ? '0' : "$totalPrice",
