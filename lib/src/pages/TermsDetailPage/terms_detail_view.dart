...
class TermsDetailView extends StatefulWidget {
  TermsDetailView({Key? key}) : super(key: key);

  @override
  _TermsDetailViewState createState() => _TermsDetailViewState();
}

class _TermsDetailViewState extends State<TermsDetailView> {
  ...
  void initState() {
    ...
    _termsController.text = _storeModel.profile!["termsAndConditions"] ?? "";
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          TermsDetailPageString.appbarTitle,
        ...
        children: [
          ...
          Expanded(
            child: KeicyTextFormField(
              ...
              hintText: TermsDetailPageString.termsHint,
              border: Border.all(color: Colors.grey.withOpacity(0.6)),
              errorBorder: Border.all(color: Colors.red.withOpacity(0.6)),
              contentHorizontalPadding: widthDp * 10,
              contentVerticalPadding: heightDp * 8,
              borderRadius: heightDp * 8,
              onChangeHandler: (input) => _storeModel.profile!["termsAndConditions"] = input.trim(),
              maxLines: null,
              keyboardType: TextInputType.multiline,
              textInputAction: TextInputAction.newline,
              // validatorHandler: (input) => input.isEmpty
              //     ? RegisterStorePageString.termsValidate1
              //     : input.length != 10
              //         ? RegisterStorePageString.termsValidate2
              //         : null,
              onSaveHandler: (input) => _storeModel.profile!["termsAndConditions"] = input.trim(),
