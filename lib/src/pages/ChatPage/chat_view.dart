...
import 'package:trapp/src/pages/InvoicePage/index.dart';
...
class _ChatViewState extends State<ChatView> {
  ...
  void initState() {
    ...
    myUserId = AuthProvider.of(context).authState.storeModel!.id;
  ...
  void _liveInHandler() async {
    ...
    await _chatProvider!.updateChatRoom(chatRoomModel: chatRoomModel, changeUpdateAt: false);
  }
  ...
  void _updateChatRoom(ChatModel chatModel, Map<String, dynamic> result) {
    ...
    if (fcmTokenList.isNotEmpty) {
      KeicyFCMForMobile.sendMessage(
        "A new message from ${AuthProvider.of(context).authState.storeModel!.name}",
    ...
    if (!crmBuild) {
      if (widget.operation != null) {
        Future.delayed(
          ...
          _additionalComponentsHandlerWithResult(widget.operation);
        });
      }
      crmBuild = true;
    }
  ...
  void _additionalComponentsHandler() async {
    ...
    _additionalComponentsHandlerWithResult(result);
  }

  _additionalComponentsHandlerWithResult(String? result) async {
    if (_chatRoomModel!.isBlocked!) return;
    ...
    switch (result) {
      case "Invoice":
        NormalAskDialog.show(
          context,
          title: "Invoice",
          content: "Complete your deal via chat. Generate an invoice with the products and services that your customer want.",
          callback: () async {
            // var result = await Navigator.of(context).push(
            //   MaterialPageRoute(
            //     builder: (BuildContext context) => GeneratePaymentLinkPage(
            //       userData: _chatRoomModel!.userData,
            //       storeData: AuthProvider.of(context).authState.userData!["storeData"],
            //     ),
            //   ),
            // );
            var result1;
            if (widget.chatRoomType == ChatRoomTypes.b2c) {
              result1 = await Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (BuildContext context) => InvoicePage(
                    userModel: UserModel.fromJson(_userData!),
                    changeCustomer: false,
                  ),
                ),
              );
              if (result1 != null) {
                OrderModel orderModel = result1;
                _sendMessageHandler(
                  ...
                  additionalData: {
                    ...
                    "userId": orderModel.userModel!.id,
                    "storeId": orderModel.storeModel!.id,
                    "totalQuantity": orderModel.paymentDetail!.totalQuantity,
                    "toPay": orderModel.paymentDetail!.toPay,
                  },
                );
              }
            } else if (widget.chatRoomType == ChatRoomTypes.b2b) {
              ...
              _b2bOrderModel.businessStoreModel = StoreModel.fromJson(_userData!);
              ...
              if (result1 != null) {
                B2BOrderModel? b2bOrderModel = result1;
                _sendMessageHandler(
                  messageType: MessageType.b2bInvoice,
                  message: "B2B Invoice",
                  additionalData: {
                    "messageType": MessageType.invoice,
                    "orderId": b2bOrderModel!.orderId,
                    "myStoreId": b2bOrderModel.myStoreModel!.id,
                    "businessStoreId": b2bOrderModel.businessStoreModel!.id,
                    "totalQuantity": b2bOrderModel.paymentDetail!.totalQuantity,
                    "toPay": b2bOrderModel.paymentDetail!.toPay,
                  },
                );
              }
            }
          ...
          MaterialPageRoute(
            builder: (BuildContext context) => CouponDetailPage(
              ...
              couponModel: CouponModel(
                eligibility: widget.chatRoomType == ChatRoomTypes.b2c
                    ? AppConfig.eligibilityForCoupon[0]["value"]
                    : widget.chatRoomType == ChatRoomTypes.b2b
                        ? AppConfig.eligibilityForCoupon[1]["value"]
                        : "",
                customerEligibility: widget.chatRoomType == ChatRoomTypes.b2c
                    ? AppConfig.customerEligibilityForCoupon[1]["value"]
                    : AppConfig.customerEligibilityForCoupon[0]["value"],
                specificCustomers: widget.chatRoomType == ChatRoomTypes.b2c ? [_userData] : [],
                businessEligibility: widget.chatRoomType == ChatRoomTypes.b2b
                    ? AppConfig.businessEligibilityForCoupon[1]["value"]
                    : AppConfig.businessEligibilityForCoupon[0]["value"],
                specificBusinesses: widget.chatRoomType == ChatRoomTypes.b2b ? [_userData] : [],
              ),
            ),
          ),
        );
        if (result1 != null && result1["isUpdated"] && result1["couponModel"] != null) {
          CouponModel? couponModel = result1["couponModel"];

          _sendMessageHandler(
            messageType: MessageType.coupon,
            message: "Coupon",
            additionalData: {
              "messageType": MessageType.invoice,
              "couponId": couponModel!.id,
              "discountCode": couponModel.discountCode,
              "discountType": couponModel.discountType,
              "discountData": couponModel.toJson()["discountData"],
            },
          );
        }
        break;
