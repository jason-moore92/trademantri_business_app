...
import 'package:trapp/src/states/customer_recent_orders_state.dart';

class CustomerRecentOrdersProvider extends ChangeNotifier {
  static CustomerRecentOrdersProvider of(BuildContext context, {bool listen = false}) =>
      Provider.of<CustomerRecentOrdersProvider>(context, listen: listen);

  CustomerRecentOrdersState _state = CustomerRecentOrdersState();
  CustomerRecentOrdersState get state => _state;

  void setCustomerRecentOrdersState(CustomerRecentOrdersState state, {bool isNotifiable = true}) {
    if (_state != state) {
      _state = state;
